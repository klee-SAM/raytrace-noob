...
// funny
vec3 DistriTracer::getRayColorImpl(const Ray& ray) const
{
    return raytrace(ray, Interval(0.005f, CONSTANTS::MAX), 0U);
}

// Ensures helper functions have internal linkage, meaning that
// the linker will not complain about multiple definitions when
// DistriTracer.h is included somewhere else
namespace {
...
// This is for cases where all sorted hits for a ray are needed.
...
vec3 DistriTracer::raytrace(const Ray& ray, const Interval& interval, uint recursiveDepth) const
{
    Hit rec;
    vec3 clr = vec3(0.0f);
...
vec3 DistriTracer::getReflectedColor(const Ray &ray, IntParams args, uint recursions) const
{
    auto &rec = args.rec;
    auto &interval = args.interval;

    vec3 reflClr = raytrace(reflectRay(ray, rec), interval, recursions);
    for (uint r = 1; r < args.rec.m->reflSamples; ++r) {
        Ray nearRay = ray;
        nearRay.setDir(glm::normalize(ray.getDir() + glm::sphericalRand(rec.m->fuzz)));
        reflClr += raytrace(reflectRay(nearRay, rec), interval, recursions);
    }
...
vec3 DistriTracer::getRefractedColor(const Ray &ray, IntParams args,
                                     uint recursions, bool back_face) const
{
    ...
    if (vec3(0.f) != ray.getDir()) {
        refrClr = raytrace(refrRay, args.interval, recursions);
...
float DistriTracer::occlusionDiffuseFactor(IntParams args, vec3 &diffuseFac, float time) const
{
    auto &rec = args.rec;
    auto &interval = args.interval;
...
constexpr auto aboveZero = [](const vec3 &clr) { 
    return clr.x > 0.f || clr.y > 0.f || clr.z > 0.f; 
};

vec3 DistriTracer::getShadowContrib(const Ray &sray, const Interval &t_int) const 
{  
    if (FULL_SHADOWS) {
        Hit srec;
...
vec3 DistriTracer::lightingFactor(const Ray &ray, IntParams args,
